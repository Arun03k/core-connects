...
from middleware.auth_middleware import rate_limit, enhanced_token_required
...
def register():
    ...
    try:
        ...
        user = result['user']
        user_name = f"{user.get('first_name', '')} {user.get('last_name', '')}".strip() or user.get('username') or 'User'
        ...
        return jsonify({
            ...
            'data': {
                ...
                'verificationToken': result['verification_token'] if not email_sent else None
            }
        }), 201
...
def reset_password():
    ...
    try:
        ...
        # Update password
        user_model.update_user(str(user['_id']), {
